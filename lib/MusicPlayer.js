...
class MusicPlayer {
  constructor(guild, client) {
    ...
    this.player.on(AudioPlayerStatus.Playing, () => {
      console.log(`[player:${this.guild.id}] now playing: ${this.current && this.current.title}`);
    });
  ...
  connect(voiceChannel) {
    ...
    this.connection.on(VoiceConnectionStatus.Disconnected, async () => {
      try {
        // Try to recover from a brief network/voice-region move.
        await Promise.race([
  ...
  async playNext() {
    let track = null;
    if (this.forceNext) {
      track = this.forceNext;
      this.forceNext = null;
    } else if (this.queue.length) {
      if (this.shuffleMode) {
        track = this.queue.splice(Math.floor(Math.random() * this.queue.length), 1)[0];
      } else {
    ...
    if (!track && this.autoplay && !this.stopping) {
      track = await this.getAutoplayTrack();
    }
    this.stopping = false;
  ...
  handleVoiceStateUpdate() {
    if (!this.connection || !this.voiceChannelId) return;
