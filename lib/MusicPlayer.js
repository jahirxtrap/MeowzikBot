...
class MusicPlayer {
  ...
  connect(voiceChannel) {
    ...
    this.connection.on(VoiceConnectionStatus.Disconnected, async () => {
      ...
      try {
        await entersState(connection, VoiceConnectionStatus.Ready, RECONNECT_TIMEOUT_MS);
      } catch {
        if (this.connection !== connection) return;
        console.error(
          `[player:${this.guild.id}] voice reconnect stalled in ${connection.state.status}; dropping it`,
        );
        this.handleDisconnected();
      }
    });
  ...
  async playNext() {
    ...
    if (this.failureStreak >= MAX_CONSECUTIVE_FAILURES) {
      console.error(
        `[player:${this.guild.id}] ${this.failureStreak} tracks failed in a row; stopping playback`,
      );
      this.failureStreak = 0;
      this.handleDisconnected();
      return;
    ...
    this.retryTimer = setTimeout(() => {
      this.retryTimer = null;
      this.playNext();
    }, RETRY_BACKOFF_MS * this.failureStreak);
  }
  ...
  async startTrack(track) {
    ...
    if (track.id) {
      this.lastPlayed = {id: track.id, url: track.url};
  ...
  async getAutoplayTrack() {
    const candidates = [];
    if (this.lastPlayed) candidates.push(this.lastPlayed);
    for (let i = this.seeds.length - 1; i >= 0; i -= 1) {
      const {id, url} = this.seeds[i];
      if (id && !candidates.some((c) => c.id === id)) candidates.push({id, url});
    }
    if (!candidates.length) return null;

    for (const seed of candidates.slice(0, 5)) {
      try {
        const related = await getRelated(seed, this.history);
        if (!related) continue;
  ...
  setAutoplay(value) {
    ...
    if (
      ...
      !this.current &&
      this.queue.length === 0 &&
      (this.seeds.length || this.lastPlayed)
    ) {
      this.playNext();
    }
