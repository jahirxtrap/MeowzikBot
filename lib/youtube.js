...
function baseArgs() {
  const args = [];
  if (cookiesRequired) args.push(...cookieArgs());
  if (process.env.YTDLP_ARGS) args.push(...splitArgs(process.env.YTDLP_ARGS));
  return args;
}
...
async function runYtdlp(args) {
  try {
    return await spawnYtdlp([...baseArgs(), ...args]);
  } catch (err) {
    if (cookiesRequired || !cookieArgs().length || !BOT_CHECK.test(err.message)) throw err;
    cookiesRequired = true;
    console.log('yt-dlp: YouTube asked to verify the client, retrying with cookies.');
    return spawnYtdlp([...baseArgs(), ...args]);
  }
}
