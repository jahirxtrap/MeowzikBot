...
function readYoutubeCookies(file) {
  ...
  const cookies = /^\s*[[{]/.test(raw) ? fromJson(raw) : fromNetscape(raw);
  return cookies.filter((c) => c.domain.includes('youtube.com'));
}
...
function holdsSession(file) {
  try {
    return missingSessionCookies(readYoutubeCookies(file)).length === 0;
  } catch {
    return false;
  }
}
...
function install(cookies, target, bin) {
  ...
  if (result.ok) replaceFile(staging, target);
